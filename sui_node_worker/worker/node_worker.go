...
func notifySBadSync(s Server, last_block int64) error {
	...
	msg := &SendMsg{
		Chat_id: chat_id,
		Text: fmt.Sprintf(
			"#ironfish\n\nHey! Your server: %s is not synced.\n\nYour server last_block: %d, Network last_block: %d", s.Ip, s.Tx_id, last_block,
		) + "\n\n-----------------------------------------------------------" + fmt.Sprintf(
			"\n\nЙоу! Твой сервер: %s отстал от сети.\n\nПоследний блок на этом сервере: %d, Последний блок в сети: %d", s.Ip, s.Tx_id, last_block,
		),
		Parse_mode: "",
	}
